...
void LWIPTCPModel::onEntryFunction(s2e::S2EExecutionState *state, uint64_t pc) {
    g_s2e->getDebugStream(state) << "ENTERING tcp_input.\n";
    // Symbolize header
    bool err = false;
    auto tcpHdrPtr = scd->readAndConcretizeMemory64(state, inputTcpHdr + loadbase, err, "Read TCP input");
    if (err) {
        g_s2e->getDebugStream(state) << "Fatal: cannot get input " << s2e::hexval(inputTcpHdr)
                                     << " at onEntryFunction.\n";
        exit(-1);
    }
    auto ipAddrLastBPtr = inputIpLastBHdr + loadbase;
...
//        assert(scd->addConstraint(state, optConstaints, "option constraints") && "add option constraints failed.\n");
    ...
    // Avoid symbolizing other field in the buffer
    uint64_t mBuffPtr;
    if (scd->readRegister64(state, R_EBX, &mBuffPtr, "onEntryFunction, get pbuf.")) {
        scd->disallowedGWRanges.emplace_back(mBuffPtr, mBuffPtr + 24 - 1, 0);
        g_s2e->getDebugStream(state) << "Input address range 0: " << s2e::hexval(mBuffPtr) << " - "
                                     << s2e::hexval(mBuffPtr + 24 - 1) << "\n";
        auto dataPtr = scd->readAndConcretizeMemory64(state, mBuffPtr + 0x8, err, "get pbuf->payload");
        if (!err) {
            auto dataLen = scd->readAndConcretizeMemory32(state, mBuffPtr + 0x12, err, "get pbuf->len");
            if (!err) {
                ...
                g_s2e->getDebugStream(state) << "Error: cannot get pbuf->len.\n";
            }
        } else {
            g_s2e->getDebugStream(state) << "Error: cannot get pbuf->payload.\n";
        }
    } else {
        g_s2e->getDebugStream(state) << "onEntryFunction, cannot get rbx for pbuf.\n";
    }
    ...
    scd->makeSymbolic(state, 0x55886084d2a8, 1, "victim_state", &victimStateSym, &victimStateSymName);
    ...
    scd->makeSymbolic(state, 0x55886084d3a8, 1, "attacker_state", &attackerStateSym, &attackerStateSymName);
    scd->addConstraint(state, klee::NotExpr::create(klee::EqExpr::create(
            attackerStateSym[0],
    ...
    scd->makeSymbolic(state, 0x5588608441e8, 1, "listen_state", &listenStateSym, &listenStateSymName);
...
s2e::plugins::GlobalWrite2 LWIPTCPModel::onOutputFunction(s2e::S2EExecutionState *state, uint64_t pc) {
    uint64_t mbuf;
    if (!scd->readRegister64(state, R_EDI, &mbuf, "onOutputFunction, read rdi.")) {
        g_s2e->getDebugStream(state) << "Error: cannot get rdi at onOutputFunction.\n";
        return {};
    }
    bool err = false;
    auto tcpHdrPtr = scd->readAndConcretizeMemory64(state, mbuf + 0x8, err, "Read tcphdr on output");
    if (err) {
        g_s2e->getDebugStream(state) << "Error: cannot get tcphdr at output with pc=" << s2e::hexval(pc) << "\n";
        exit(-1);
    }
...
uint8_t LWIPTCPModel::getNewStateOwner(s2e::S2EExecutionState *oldState, s2e::S2EExecutionState *newState,
...
LWIPTCPModel::LWIPTCPModel(s2e::plugins::Propagator *scd) : ProtocolModel(scd) {
    entryPoints.insert(0xdfa2 + loadbase);
    outputPoints.insert(0x19af3 + loadbase);
    terminatePoints.insert(0xe73d + loadbase);
    terminatePoints.insert(0xe974 + loadbase);
    terminatePoints.insert(0xe047 + loadbase);
    terminatePoints.insert(0xe085 + loadbase);
    terminatePoints.insert(0xe08f + loadbase);
    terminatePoints.insert(0xeac5 + loadbase);
